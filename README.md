...
- **Stochastic optimizer** (Recommendation tab → "Stochastic" mode): the
  ...
  snapshot
- **Data Quality tab**: runs automatically on every upload, checking your
  sales history for problems that would quietly corrupt a long-range trend
  or seasonal model — timeline gaps (months where every SKU shows zero
  orders, almost always a data export issue, not real demand collapse),
  SKU discontinuity (a product that goes quiet for months then comes back,
  possibly a relaunch or a reused SKU code rather than continuous history),
  and statistical anomalies (demand spikes/collapses far outside a SKU's
  normal range, e.g. a one-off event that would skew an average). This is
  advisory, not a blocker — the rest of the tool works normally regardless,
  it only matters once you start trusting multi-year seasonal patterns
- **Premium visual theme, v2**: left-rail icon navigation (replacing the
  earlier top-tab layout) and a restrained orange/white/charcoal/near-black
  palette — accent color reserved for the active nav item and 2-3 key
  elements per screen, everything else monochrome. Data upload and
  assumptions moved to a dedicated "Data & Settings" rail item instead of
  living in the sidebar (the sidebar is now nav-only)
- **Persistent weekly data** — upload Sales History, Inventory, and Invoices
  once (e.g. every Monday) and the tool remembers them across sessions —
  no re-upload needed until you explicitly replace them. The Setup page
  shows exactly when the data was last uploaded and flags it if it's
  more than a week old
- **Manifest-scoped views** — once you upload a Send-to-Amazon manifest,
  the Recommendation tab and Dashboard automatically narrow to just the
  SKUs on that manifest (with a "Clear scope" button to return to your
  full catalog) — so the tool only talks about the shipment you're
  actually working on, not your entire historical catalog
- **Box-level packing** — goes one level past the regional split. Once a
  region's unit allocation is known (e.g. 180 units to West), the tool
  reads the real units-per-box and box dimensions/weight from your
  manifest and computes the actual box breakdown (full boxes + partial
  last box), instead of leaving the box/case-pack columns blank for
  manual entry. SKUs with no box spec on the manifest fall back to an
  editable default, clearly flagged as a fallback rather than a real number
...
```
app.py                      Streamlit app -- left-rail page router + all page bodies
modules/
  ...
  data_continuity_checker.py  Flags timeline gaps, SKU discontinuity, and
                               anomalous periods before trusting long-range
                               trends (run automatically on upload)
  data_persistence.py         Saves uploaded data to disk so it survives
                               across sessions (weekly-refresh workflow)
  box_packing_optimizer.py    Box-level packing: units -> boxes using real
                               manifest box specs (units/box, dimensions,
                               weight), with editable fallback for SKUs
                               missing box info
  theme.py                    Premium orange/white/charcoal CSS theme,
                               injected via st.markdown -- visual layer only
...
## Roadmap note: multi-year seasonality (in progress)

The Data Quality tab is the first piece of a larger plan to use multi-year
history (5 years, when ready) for real seasonal decomposition — separating
genuine trend, seasonality, and noise, rather than treating every
historical window as an interchangeable data point the way the current
Stochastic mode does. This requires the continuity checks above to pass
first (or for flagged issues to be manually reviewed and resolved) before
building trend/seasonality math on top — feeding 5 years of data with
unverified SKU continuity or undetected gaps straight into a seasonal
model would produce a confident-looking number that's quietly wrong.

## Persistent data: how it actually works

Uploaded files are saved to a `persisted_data/` folder on the server's
disk (excluded from git via `.gitignore`). This is appropriate for a
single-operator weekly-refresh workflow at $0 infrastructure cost — it is
**not** a database and **not** per-user storage. If this tool is ever used
by multiple people with different data simultaneously, this model breaks
down (everyone would share the same persisted files) — that would need a
real per-user storage layer, which is a different and larger build than
what's here now.

On Streamlit Community Cloud specifically: the free tier's filesystem is
not guaranteed to persist forever across app restarts/redeploys (it can
reset on a redeploy or after extended inactivity) — so treat "persisted"
as "survives between your normal sessions this week," not "permanent
storage you never have to think about." If data unexpectedly disappears
after a long gap, that's the free-tier filesystem resetting, not a bug —
just re-upload on your next Monday refresh as usual.

## Design scope note

The visual theme borrows the DESIGN LANGUAGE of premium SaaS dashboards
(soft-shadow cards, rounded corners, depth via layered shadows, warm
gradient accents) in Virventures' orange/white/charcoal brand colors. It
does not attempt literal 3D-rendered objects or photographed-device
mockups — that's product photography/3D rendering, a different medium
than a live data app, and not achievable (or appropriate) inside a
Streamlit dashboard that needs to stay functional and fast.
