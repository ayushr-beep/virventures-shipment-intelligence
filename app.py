...
st.sidebar.markdown(
    ...
    <div style="display:flex; align-items:center; gap:0.6rem; margin-bottom:0.2rem;">
        <div style="width:34px; height:34px; border-radius:9px;
                    background: linear-gradient(135deg, #D2691E 0%, #B85A18 100%);
                    display:flex; align-items:center; justify-content:center;
                    font-size:1.1rem; box-shadow: 0 3px 8px rgba(210,105,30,0.3);">📦</div>
        <div style="font-family: Georgia, serif; font-size:1.15rem; font-weight:700; color:#252830;">
            Shipment Intelligence
        </div>
    </div>
    <div style="font-size:0.8rem; color:#6B7280; margin-bottom:1rem;">Virventures · Demand-weighted FBA placement</div>
    """,
...
st.markdown(
    """
    <div style="background: linear-gradient(135deg, #FDF1E7 0%, #FBE4CE 100%);
                border-radius: 20px; padding: 1.6rem 2rem; margin-bottom: 1.2rem;
                border: 1px solid #EDE6DC; box-shadow: 0 2px 8px rgba(37,40,48,0.04), 0 8px 24px rgba(37,40,48,0.06);">
        <div style="font-size:0.78rem; font-weight:700; letter-spacing:0.08em; text-transform:uppercase; color:#B85A18;">
            VIRVENTURES &nbsp;·&nbsp; SHIPMENT INTELLIGENCE
        </div>
        <div style="font-family: Georgia, serif; font-size: 2rem; font-weight: 700; color:#252830; margin-top:0.2rem;">
...
# Tabs
# ---------------------------------------------------------------------------
tab_dash, tab_quality, tab_recommend, tab_manifest, tab_export = st.tabs(
    ["📊 Dashboard", "🔍 Data Quality", "🎯 Recommendation", "📦 Shipment Plan (Manifest Upload)", "📁 Export"]
)
...
with tab_dash:
    if continuity_report["total_span_months"] >= 12 and not continuity_report["is_ready_for_seasonality_modeling"]:
        ...
        st.warning(
            ...
            f"{reasons_str}. See the **Data Quality** tab for details before relying on seasonal patterns."
        )
...
with tab_quality:
    ...
    with st.expander("SKU continuity — products that went quiet for a while, then came back"):
        ...
        if len(sku_gaps_flagged) > 0:
            st.dataframe(sku_gaps_flagged, use_container_width=True, hide_index=True)
        else:
            st.caption("None found.")
        with st.popover("View all SKUs (including ones with no issues)"):
            st.dataframe(continuity_report["sku_gap_detail"], use_container_width=True, hide_index=True)
    ...
    with st.expander("Anomalous periods — months far outside a SKU's normal range"):
        ...
        if len(continuity_report["anomaly_detail"]) > 0:
            st.dataframe(continuity_report["anomaly_detail"], use_container_width=True, hide_index=True)
        else:
            st.caption("None found.")
