...
if active_page == "dashboard":
    ...
    _manifest_scope_dash = st.session_state.get("_manifest_sku_scope")
    matrix_source_df = demand_profile
    if _manifest_scope_dash:
        scoped_df = demand_profile[demand_profile["SKU"].isin(_manifest_scope_dash)]
        if len(scoped_df) > 0:
            matrix_source_df = scoped_df
            st.caption(
                f"📦 Scoped to the {len(scoped_df)} SKU(s) on your uploaded manifest "
                f"({st.session_state.get('_manifest_filename', 'uploaded file')})."
            )
    ...
    matrix_df = matrix_source_df[["SKU"] + matrix_cols].copy()
...
if active_page == "recommend":
    ...
    manifest_scope = st.session_state.get("_manifest_sku_scope")
    full_sku_options = demand_profile["SKU"].tolist()

    if manifest_scope:
        scoped_options = [s for s in full_sku_options if s in manifest_scope]
        manifest_name = st.session_state.get("_manifest_filename", "your uploaded manifest")
        scope_col1, scope_col2 = st.columns([5, 1])
        scope_col1.info(
            f"📦 Scoped to **{manifest_name}** — showing only the {len(scoped_options)} SKU(s) from that "
            f"manifest that have sales history. Clear the scope to see your full catalog again."
        )
        if scope_col2.button("Clear scope", key="_clear_manifest_scope"):
            del st.session_state["_manifest_sku_scope"]
            st.rerun()
        sku_options = scoped_options if scoped_options else full_sku_options
        if not scoped_options:
            st.warning("None of the manifest's SKUs have sales history — showing the full catalog instead.")
    else:
        sku_options = full_sku_options
...
if active_page == "manifest":
    ...
    if manifest_file is not None:
        try:
            # Read the upload's bytes ONCE, then hand each parse call its own
            # independent BytesIO buffer. Re-using the same Streamlit
            # UploadedFile object across two separate pd.ExcelFile() calls --
            # even with seek(0) in between -- breaks, because openpyxl's
            # internal zip/XML reader doesn't cleanly support being re-driven
            # from the same file-like object a second time. This was the
            # actual cause of the AttributeError seen on manifest upload.
        ...
        if parsed_manifest is not None:
            ...
            # Scope the rest of the app (Recommendation tab's SKU picker, etc.)
            # to ONLY the SKUs on this manifest -- this is what was asked for:
            # once a manifest is uploaded, every other view should talk about
            # these SKUs specifically, not the full historical catalog.
            st.session_state["_manifest_sku_scope"] = sorted(parsed_manifest["Merchant SKU"].astype(str).unique().tolist())
            st.session_state["_manifest_filename"] = getattr(manifest_file, "name", "uploaded manifest")
            ...
            if len(summary_df) > 0:
                ...
                st.markdown("###### Per-Region Files — Box Counts Pre-Filled")
                st.caption(
                    "Same Send-to-Amazon column layout, but Units per box and Number of boxes are now "
                    "computed and filled in — not left blank."
                )
                ...
                for i, region in enumerate(REGIONS):
                    ...
                    if len(box_export) == 0:
                        ...
                        continue
                    box_csv_bytes = box_export.to_csv(index=False).encode("utf-8")
                    box_export_cols[i].download_button(
                        f"⬇️ {region} — {int(box_export['Number of boxes'].sum())} boxes",
                        data=box_csv_bytes,
                        file_name=f"manifest_{region.lower()}_with_boxes_{datetime.now().strftime('%Y%m%d')}.csv",
                        mime="text/csv",
